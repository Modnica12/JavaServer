...
public class Database
{
	...
	{
		return connection.prepareStatement("SHOW DATABASES LIKE 'AUTHDB';").executeUpdate() == 1;
	}
	...
	public void connect() throws ClassNotFoundException
	{
		...
		System.out.println("Connecting to auth database...");
		try
		{
			boolean           dbExisted = false;
			Connection        conn      = DriverManager.getConnection(DB_URL, username, password);
			PreparedStatement stmt      = conn.prepareStatement("CREATE DATABASE IF NOT EXISTS AUTHDB;");
			stmt.execute();
			PreparedStatement stmtUTF      =
					conn.prepareStatement("ALTER DATABASE AUTHDB CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;");
			stmtUTF.execute();
			statement = conn.createStatement();
			connection = conn;
			System.out.println("Successful connection to authentication database.");
